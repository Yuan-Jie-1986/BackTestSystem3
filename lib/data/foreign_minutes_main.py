...
for a in asset:
    df_raw_1 = pd.read_csv(os.path.join(raw_path, a, a + 'c1.csv'), index_col=0, parse_dates=True)
    df_vol_1 = df_raw_1[['VOLUME']].copy()
    df_vol_1.rename(columns={'VOLUME': a + 'c1'}, inplace=True)

    df_raw_2 = pd.read_csv(os.path.join(raw_path, a, a + 'c2.csv'), index_col=0, parse_dates=True)
    df_vol_2 = df_raw_2[['VOLUME']].copy()
    df_vol_2.rename(columns={'VOLUME': a + 'c2'}, inplace=True)

    df_raw_3 = pd.read_csv(os.path.join(raw_path, a, a + 'c3.csv'), index_col=0, parse_dates=True)
    df_vol_3 = df_raw_3[['VOLUME']].copy()
    df_vol_3.rename(columns={'VOLUME': a + 'c3'}, inplace=True)

    df_raw_4 = pd.read_csv(os.path.join(raw_path, a, a + 'c4.csv'), index_col=0, parse_dates=True)
    df_vol_4 = df_raw_4[['VOLUME']].copy()
    df_vol_4.rename(columns={'VOLUME': a + 'c4'}, inplace=True)

    df_raw_5 = pd.read_csv(os.path.join(raw_path, a, a + 'c5.csv'), index_col=0, parse_dates=True)
    df_vol_5 = df_raw_5[['VOLUME']].copy()
    df_vol_5.rename(columns={'VOLUME': a + 'c5'}, inplace=True)

    df_raw_6 = pd.read_csv(os.path.join(raw_path, a, a + 'c6.csv'), index_col=0, parse_dates=True)
